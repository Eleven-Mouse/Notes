...
在博客项目里，Doris 最适合放在后台报表后面：**保存 Spark 统计后的 PV、UV、热门文章、来源分布、慢请求等指标，让后台页面可以快速查询。**

不要用 Doris 替代博客主库。文章、用户、评论这些业务数据仍然应该放在 MySQL 等事务型数据库中。
...
| 表 | 用途 |
| --- | --- |
| blog_site_daily_stat | 站点每日 PV、UV、访问次数 |
| blog_article_daily_stat | 每篇文章每日 PV、UV |
| blog_referer_daily_stat | 来源分布 |
| blog_status_daily_stat | 状态码分布 |
| blog_slow_request_stat | 慢请求统计 |
...
```text
Spark 统计博客访问日志 -> Doris 指标表 -> 博客后台查询日报
```
